...
(function ($) {
    ...
    $.fn.horizon = function (options, i) {
        if (options === 'scrollLeft') {
            ...
            $(".menu-buton").on('click', function(e) {
                var evt = window.event || e;
                scrollTo(evt.srcElement.id, $.fn.horizon.defaults.scrollDuration);
            });
...
// http://stackoverflow.com/questions/8790752/callback-of-animate-gets-called-twice-jquery
    var scrollTo = function (index, speed) {
        ...
        console.log('Scroll to: ' + $.fn.horizon.defaults.scrollDuration);
        ...
        console.log("index after move : "+index);
        if (index == 0) {
            $('.horizon-prev').hide();
            $('.horizon-next').show();
